...
$cfg = [
	...
	'queries'       => [
		...
		'CREATE TABLE IF NOT EXISTS `' . PREFIX . '_components_fields_data` (
			...
			KEY `type` (`type`)
		) COMMENT=\'Данные дополнительных полей компонентов\';',

		'CREATE TABLE IF NOT EXISTS `' . PREFIX . '_components_fields_list` (
		...
		) ROW_FORMAT=COMPACT COMMENT=\'Таблица для хранения списка допполей компонентов\';',

		'CREATE TABLE IF NOT EXISTS `' . PREFIX . '_components_fields_types` (
			`id` tinyint(3) NOT NULL AUTO_INCREMENT,
			`type` char(4) NOT NULL,
			`description` varchar(255) NOT NULL,
			UNIQUE KEY `id` (`id`),
			UNIQUE KEY `type` (`type`)
		) AUTO_INCREMENT=13 COMMENT=\'Таблица для хранения типов допполей\';',


		'REPLACE INTO `' . PREFIX . '_components_fields_types` (`id`, `type`, `description`) VALUES
...
function installer() {
	...
	if ($queries) {
		...
		foreach ($cfg['queries'] as $qq) {
			$queriesTxt .= '<textarea readonly>' . str_replace("\t\t", '', $qq) . '</textarea>';
		}
	}
